...
pub fn join<R>(client: &R, request: &JoinRequest<'_>) -> Result<JoinResponse, JoinError<R::Error>>
where
    R: SlackWebRequestSender,
{
    let params = vec![
...
pub fn kick<R>(client: &R, request: &KickRequest<'_>) -> Result<KickResponse, KickError<R::Error>>
where
    R: SlackWebRequestSender,
{
    let params = vec![
